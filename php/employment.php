    ...
    if (preg_match('/(\d{2}\/\d{2}\/\d{4})/', $datetime_str, $matches)) {
        ...
        if ($date_obj) {
            ...
            if ($days <= 1) {
                ...
                return '¡Nuevo!'; // <<== Nombre final implementado
...
$puestos_encontrados_count = 0;
    ...
    if ($html_content === false) {
       ...
       $color_y_negrita = 'style="color: #1a73e8; font-weight: bold; font-size: 1.1em;"';
        ...
        echo '<p><span ' . $color_y_negrita . '>' . $puestos_encontrados_count . '</span> nuevas oportunidades de empleo te están esperando:</p>';
